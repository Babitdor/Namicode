...
class HarborSandbox(SandboxBackendProtocol):
    ...
    def execute(
        ...
        **kwargs,
        ...
        """Execute a bash command — delegates to async."""
        try:
            import asyncio

            loop = asyncio.get_event_loop()
            if loop.is_running():
                # Already in an async context (e.g. threadpool inside deepagents) —
                # submit to the running loop and wait for the result.
                future = asyncio.run_coroutine_threadsafe(self.aexecute(command, **kwargs), loop)
                return future.result()
            else:
                return loop.run_until_complete(self.aexecute(command, **kwargs))
        except RuntimeError:
            return asyncio.run(self.aexecute(command, **kwargs))
        except RuntimeError:
    ...
    # deepagents' BaseSandbox.write/read route through upload_files/
    # download_files (NOT awrite/aread), so a backend that omits these raises
    # NotImplementedError on every file write. We implement them here with the
    # same base64-over-shell approach as awrite/aread (no python3 assumed).

    @staticmethod
    def _build_upload_cmd(file_path: str, content: bytes) -> str:
        content_b64 = base64.b64encode(content).decode("ascii")
        safe_path = shlex.quote(file_path)
        return (
            f"parent_dir=$(dirname {safe_path}); "
            f'mkdir -p "$parent_dir" 2>/dev/null; '
            f"echo '{content_b64}' | base64 -d > {safe_path}"
        )

    @staticmethod
    def _build_download_cmd(file_path: str) -> str:
        safe_path = shlex.quote(file_path)
        # base64-encode the file so binary content survives the text channel.
        # Sentinel on missing file → mapped to a `file_not_found` error below.
        return (
            f'if [ ! -f {safe_path} ]; then echo "__NOVA_NOT_FOUND__"; exit 1; fi; '
            f"base64 {safe_path}"
        )

    @staticmethod
    def _parse_download(file_path: str, result: ExecuteResponse) -> FileDownloadResponse:
        if result.exit_code != 0 or "__NOVA_NOT_FOUND__" in (result.output or ""):
            return FileDownloadResponse(path=file_path, content=None, error="file_not_found")
        try:
            # b64decode ignores embedded newlines from `base64`'s line wrapping.
            content = base64.b64decode(result.output.strip())
        except Exception:  # noqa: BLE001
            return FileDownloadResponse(path=file_path, content=None, error="decode_error")
        return FileDownloadResponse(path=file_path, content=content, error=None)

    @staticmethod
    def _upload_response(file_path: str, result: ExecuteResponse) -> FileUploadResponse:
        if result.exit_code != 0:
            err = (result.output or "").strip() or f"Failed to write '{file_path}'"
            return FileUploadResponse(path=file_path, error=err)
        return FileUploadResponse(path=file_path, error=None)

    def upload_files(self, files: list[tuple[str, bytes]]) -> list[FileUploadResponse]:
        """Write (overwrite) files in the sandbox via base64 over the shell."""
        return [
            self._upload_response(
                file_path, self.execute(self._build_upload_cmd(file_path, content))
            )
            for file_path, content in files
        ]

    async def aupload_files(self, files: list[tuple[str, bytes]]) -> list[FileUploadResponse]:
        """Async variant of :meth:`upload_files`."""
        responses: list[FileUploadResponse] = []
        for file_path, content in files:
            result = await self.aexecute(self._build_upload_cmd(file_path, content))
            responses.append(self._upload_response(file_path, result))
        return responses

    def download_files(self, paths: list[str]) -> list[FileDownloadResponse]:
        """Read files from the sandbox via base64 over the shell."""
        return [self._parse_download(p, self.execute(self._build_download_cmd(p))) for p in paths]

    async def adownload_files(self, paths: list[str]) -> list[FileDownloadResponse]:
        """Async variant of :meth:`download_files`."""
        responses: list[FileDownloadResponse] = []
        for p in paths:
            result = await self.aexecute(self._build_download_cmd(p))
            responses.append(self._parse_download(p, result))
        return responses
