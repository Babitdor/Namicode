...
print("\n=== Manual trace ===")
print(f"skill_dir_path from backend: {skill_dir_path!r}")

# This is what the code does
directory_name = os.path.basename(skill_dir_path.rstrip("/\\"))
print(f"directory_name = os.path.basename(skill_dir_path.rstrip('/\\\\')): {directory_name!r}")

# Now simulate _parse_skill_metadata normalization
normalized_dir_name = directory_name.rstrip("/\\")
print(f"normalized_dir_name = directory_name.rstrip('/\\\\'): {normalized_dir_name!r}")

if os.sep in normalized_dir_name or "/" in normalized_dir_name:
    print("Found separator, calling os.path.basename again")
    normalized_dir_name = os.path.basename(normalized_dir_name)
    print(f"After os.path.basename: {normalized_dir_name!r}")
else:
    print("No separator found")

print(f"\nFinal normalized_dir_name: {normalized_dir_name!r}")
print("Expected: 'algorithmic-art'")
