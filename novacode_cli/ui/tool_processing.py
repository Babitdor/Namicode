...
def display_tool_result_preview(
    ...
    available_width = panel_width - 10
    
    # Truncate preview if too long
    if len(preview) > available_width:
        max_preview_len = available_width - 3
        preview = preview[:max_preview_len] + "..."
    
    # Build beautiful result panel with visual hierarchy
    result_icon = "✗" if is_err else "✓"
    result_color = "red" if is_err else "green"
    
    body_lines = [
        "",  # Top padding
        f"  {result_icon}  [bold]{preview}[/bold]",  # Result with icon
        "",  # Bottom padding
    ]
    
    # Create beautiful title with decorative elements
    title_text = f"[bold {result_color}]◆[/bold {result_color}] [bold]Tool Result[/bold] [bold {result_color}]◆[/bold {result_color}]"
...
def _handle_task_completion(
    ...
    """
    from novacode_cli.config.config import get_agent_color
    from novacode_cli.ui.streaming import format_condensed_activity
    from novacode_cli.ui.subagent_tracking import format_duration, get_status_icon
