"""Task execution — rich-console renderer for agent stream events.

``execute_task`` consumes the event stream from
:func:`novacode_cli.core.agent_loop.iterate_agent_events` and renders each
event to the ``rich`` console with a status spinner, tool panels, file-op
previews, and subagent tracking.

This is a thin adapter — the shared streaming/dedup/interrupt logic lives in
``iterate_agent_events``.  Bug fixes and feature additions belong *there* so
the Textual TUI (``novacode_cli.agent_stream.run_agent_stream``) benefits too.
"""
