...
from nova_deepagents.backends.protocol import SandboxBackendProtocol
...
async def simple_cli(
    ...
    async def _save_session(*, silent: bool = False) -> bool:
        ...
        try:
            ...
            if messages:
                ...
                try:
                    from nova_deepagents.middleware.shared_memory import save_shared_memory
    ...
    while True:
        ...
        if session_state.pending_plan_mode_sync:
            ...
            try:
                from novacode_cli.agents.core_agent import set_agent_plan_mode_state
...
async def main(
    ...
    if _SQLITE_CHECKPOINTER_AVAILABLE:
        checkpoints_dir = settings.nova_dir / "checkpoints"
        ...
        checkpointer = _AsyncSqliteSaver.from_conn_string(
            str(checkpoints_dir / "nova_checkpoints.db")
        )
    ...
    if continue_session:
        ...
        if result:
            ...
            # Load NOVA.md for continuation prompt
            nova_md_content = load_Nova_md(project_root)
            ...
            if session_data.shared_memory:
                from nova_deepagents.middleware.shared_memory import restore_shared_memory
