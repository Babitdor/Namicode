...
``run_agent_stream`` runs the agent and yields :mod:`novacode_cli.ui_events`
describing what happened, instead of printing. Any front-end (the new Textual
TUI, tests, a future headless consumer) can drive it; the legacy ``rich`` UI
keeps using :func:`novacode_cli.ui.execution.execute_task` unchanged during the
migration.
...
This is a faithful port of the streaming/dedup/interrupt logic in
``execute_task`` with rendering removed. It intentionally reuses the same
trackers and helpers so behavior stays consistent.
"""
...
async def run_agent_stream(  # noqa: C901, PLR0912, PLR0915
    user_input: str,
    agent,
