...
including GLM-5, GPT-4, Claude-3, and others. Uses dyNovac detection from
...
def get_model_config(model_name: str, use_dyNovac: bool = True) -> ModelConfig:
    ...
    Example:
        ...
        202752  # DyNovacally detected from Ollama
        
        >>> config = get_model_config("glm-5", use_dyNovac=False)
        >>> print(config.context_window)
    ...
    if use_dyNovac:
        try:
            from .dyNovac_context import get_model_config_dyNovac
            
            # Get dyNovac configuration
            dyNovac_config = get_model_config_dyNovac(model_name)
            
            if dyNovac_config.get("source") == "dyNovac":
                # Create ModelConfig from dyNovac data
                config = ModelConfig(
                    name=dyNovac_config["name"],
                    context_window=dyNovac_config["context_window"],
                    safe_budget=dyNovac_config["safe_budget"],
                    growth_threshold=dyNovac_config["growth_threshold"],
                    eviction_threshold=dyNovac_config["eviction_threshold"],
                    token_ratio=dyNovac_config["token_ratio"],
                    supports_streaming=True,
                    cost_per_1k_tokens=(0.0, 0.0),
                )
                logger.info(f"Using dyNovac config for {model_name}: {config.context_window:,} tokens")
                return config
...
def get_model_config_auto(model_name: Optional[str] = None, use_dyNovac: bool = True) -> ModelConfig:
    ...
    Example:
        ...
        >>> config = get_model_config_auto("glm-5", use_dyNovac=False)
    """
    if model_name:
    ...
    if detected:
        return get_model_config(detected, use_dyNovac=use_dyNovac)
