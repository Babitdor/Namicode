...
def lsp_completions(
    ...
    try:
        ...
        def sort_key(c):
            ...
            return (
                0 if name.lower().startswith(prefix.lower()) else 1,
                type_priority.get(c["type"], 5),
                name,
            )
    ...
    try:
        ...
        if len(python_files) > max_files:
            ...
            for f in python_files:
                ...
                if any(
                    f"/{pd}/" in path_str or f"\\{pd}\\" in path_str for pd in common_source_dirs
                ):
                ...
                for edit in sorted_edits:
                    ...
                    if 0 <= line_idx < len(lines):
                        ...
                        if line_content[col : col + len(old_name)] == old_name:
                            new_line = (
                                line_content[:col] + new_name + line_content[col + len(old_name) :]
                            )
...
def lsp_implementation(
    ...
    try:
        ...
        if script is None:
            return {
                "success": False,
                "error": "Failed to create Jedi script",
                "implementations": [],
            }
