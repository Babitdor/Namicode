"""Vixie integration server for NOVA CLI.

This module provides a WebSocket server that broadcasts NOVA state changes
to connected Vixie desktop pet clients, enabling real-time visual feedback.

Key Features:
- WebSocket server that broadcasts state updates
- State tracking for different NOVA phases (idle, thinking, working, etc.)
...
class NovaState(Enum):
    """Nova-Code state for Vixie visualization."""
    IDLE = "idle"
    THINKING = "thinking"
    WORKING = "working"
    SUCCESS = "success"
...
class VixieServer:
    ...
    This server broadcasts Nova-Code state changes to connected Vixie clients,
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 8765):
        ...
        self.current_state = NovaState.IDLE
        ...
        try:
            ...
            if e.errno == 10048 or "address already in use" in str(e).lower():
                logger.warning(
                    ...
                    "This is likely from another Nova instance. "
    ...
    async def broadcast_state(self, state: NovaState) -> None:
...
async def update_vixie_state(state: NovaState) -> None:
    """Update and broadcast the current Nova-Code state.
...
# State update functions for different Nova-Code phases
async def set_idle() -> None:
    """Set state to idle."""
    await update_vixie_state(NovaState.IDLE)


async def set_thinking() -> None:
    """Set state to thinking."""
    await update_vixie_state(NovaState.THINKING)


async def set_working() -> None:
    """Set state to working."""
    await update_vixie_state(NovaState.WORKING)


async def set_success() -> None:
    """Set state to success."""
    await update_vixie_state(NovaState.SUCCESS)


async def set_error() -> None:
    """Set state to error."""
    await update_vixie_state(NovaState.ERROR)


async def set_user_input_required() -> None:
    """Set state to user input required."""
    await update_vixie_state(NovaState.USER_INPUT)


async def set_planning() -> None:
    """Set state to planning."""
    await update_vixie_state(NovaState.PLANNING)
