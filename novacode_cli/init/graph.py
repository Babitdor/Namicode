...
def _coerce_str(value: Any, default: str) -> str:
    ...
    """Coerce a value to ``str``; ``None`` â†’ *default*, other types â†’ ``str(value)``."""
...
"""Edge fields that must be strings â€” dicts here cause '>' comparison crashes."""
...
"""Node fields used in arithmetic / sorting â€” a dict here raises
...
def _sanitize_node(node: dict[str, Any]) -> bool:
    ...
    # not when it's present-but-empty/None â€” so force a usable one.
...
def _sanitize_edge(edge: dict[str, Any]) -> dict[str, Any] | None:

    """Coerce one edge's fields in place (endpoints/weights, never dropped)."""

    # Guarantee a numeric weight on EVERY edge â€” NetworkX/graphify arithmetic
...
def sanitize_graph_extraction(extraction: dict[str, Any]) -> dict[str, Any]:
    ...
    We coerce every known field â€” and drop nodes with an unusable id â€” at the
...
def build_project_graph(
    ...
    try:
        ...
        console.print(
            ...
            "[yellow]graphify not installed â€” install with: "
    ...
    if not extraction.get("nodes"):
        ...
        console.print("[yellow]âš  No nodes to build graph from[/yellow]")

        return None



    # Guarantee numeric edge weights before handing off to graphify/NetworkX â€”
...
# Minimum community size. Communities with â‰¤ this many nodes are merged
...
def cluster_project_graph(
    ...
    After clustering, tiny communities (â‰¤2 nodes) are merged into their
...
def export_project_graph(
    ...
    enriched_data = _enrich_graph_json(json_path, communities, analysis, community_labels)



    # Build fast-lookup index from enriched graph

    index_path = output_dir / "graph-index.json"

    if enriched_data is not None:

        _build_graph_index(enriched_data, index_path)
...
def _enrich_graph_json(
    ...
    for node in data.get("nodes", []):
        ...
        # (graphify can emit null labels) â€” `or ""` guards `.lower()`.
...
def _build_graph_index(
    ...
    """Build a fast-lookup index from the enriched project graph.



    Produces a compact JSON file with hash maps for O(1) lookups by

    symbol name, file path, and community ID.



    Args:

        graph_data: Enriched graph data (from _enrich_graph_json output).

        output_path: Path to write the index JSON file.
    ...
    # symbol_map: symbol_name -> {community, file, connections}

    symbol_map: dict[str, dict[str, Any]] = {}

    # file_map: file_path -> {community, community_label, symbols, connections}

    file_map: dict[str, dict[str, Any]] = {}

    # Track per-file state

    file_communities: dict[str, int] = {}
    ...
    for node in nodes:
        ...
        if label:
            ...
            if edges:
                ...
                entry["connections"] = edges

            symbol_map[label] = entry



        if sf and comm is not None:

            file_communities.setdefault(sf, comm)

            file_symbols.setdefault(sf, []).append(label or nid)

            current = file_connections.get(sf, 0)

            if edges > current:

                file_connections[sf] = edges



    # Build community labels lookup

    communities_meta = meta.get("communities", [])
    ...
    for c in communities_meta:
        ...
        if cid is not None:
            ...
            comm_labels[cid] = c.get("label", f"Community {cid}")



    for sf, comm in file_communities.items():

        file_map[sf] = {

            "community": comm,

            "community_label": comm_labels.get(comm, f"Community {comm}"),

            "symbols": file_symbols.get(sf, []),

            "connections": file_connections.get(sf, 0),

        }



    # community_map: community_id -> {label, node_count, files}

    # Build file-to-community mapping in a single pass (O(n))

    community_files: dict[int, set[str]] = {}
    ...
    for n in nodes:
        ...
        if nc is not None and sf:

            community_files.setdefault(nc, set()).add(sf)
    ...
    for c in communities_meta:
        ...
        community_map[cid] = {
            ...
            "files": sorted(community_files.get(cid, set())),
...
def _show_cluster_panel(
    ...
    panel = Panel(
        ...
        title=f"[bold {COLORS['primary']}]ðŸ”— Clustering[/bold {COLORS['primary']}]",
...
def _show_analysis_panel(
    ...
    if god_node_list:
        ...
        for gn in god_node_list[:5]:
            ...
            lines.append(f"  â€¢ {gn.get('label', gn.get('id', '?'))} ({degree} edges)")
    ...
    if surprise_list:
        ...
        for sc in surprise_list[:3]:
            ...
            lines.append(f"  â€¢ {sc.get('source', '?')} â†” {sc.get('target', '?')}")
    ...
    panel = Panel(
        ...
        title=f"[bold {COLORS['primary']}]ðŸ” Analysis[/bold {COLORS['primary']}]",
...
def _show_export_panel(outputs: dict[str, Path], console: Console) -> dict[str, Any] | None:
    ...
    panel = Panel(
        ...
        title=f"[bold {COLORS['primary']}]ðŸ“¦ Export[/bold {COLORS['primary']}]",
