...
def detect_old_structure() -> list[Path]:
    ...
    Returns:
        List of agent directories in old structure (e.g., ~/.nova/agent/)
    """
    deepagents_dir = Path.home() / ".nova"
...
def migrate_agents() -> None:
    ...
    Migrates:
    - Agent directories: ~/.nova/{agent}/ → ~/.nova/agents/{agent}/
    - Skills: ~/.nova/{agent}/skills/ → ~/.nova/skills/ (global)
    """
    ...
    console.print("  1. Move agents to ~/.nova/agents/{agent_name}/")
    console.print("  2. Move per-agent skills to global ~/.nova/skills/")
    ...
    global_skills_dir = Path.home() / ".nova" / "skills"
...
def check_migration_status() -> None:
    ...
    console.print("Run 'Nova migrate' to upgrade to the new directory structure.")
