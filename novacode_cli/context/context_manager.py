...
def get_context_window_size(model_name: str, use_dyNovac: bool = True) -> int:
    ...
    if use_dyNovac and not is_cloud_api_model:
        try:
            from novacode_cli.utils.dynamic_context import get_ollama_context_length
            ...
            logger.debug(f"Dynamic context detection failed for {model_name}: {e}, falling back to hardcoded")
...
def get_dynamic_context_info(model_name: str) -> dict:
    ...
    """
    try:
        from novacode_cli.utils.dynamic_context import get_model_info
