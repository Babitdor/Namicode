...
from namicode_cli.widgets.autocomplete import (
    SLASH_COMMANDS,
    CompletionResult,
    MentionController,
...
from namicode_cli.widgets.history import HistoryManager
...
class ChatTextArea(TextArea):
    ...
    def action_select_all_text(self) -> None:
        ...
        self.selection = ((0, 0), (end_row, end_col))  # type: ignore
...
class ChatInput(Vertical):
    ...
    def on_mount(self) -> None:
        ...
        self._completion_manager = MultiCompletionManager(
            [
                SlashCommandController(SLASH_COMMANDS, self), # type: ignore
                MentionController(self, cwd=self._cwd),
            ]
        )
    ...
    def insert_completion(self, text: str) -> None:
        ...
        current_text = self._text_area.text

        if not current_text:
            # If text is empty, just set it
            self._text_area.text = text
            # Move cursor to end
            self._text_area.move_cursor((0, len(text)))
            return
        ...
        new_cursor_col = cursor_col + len(text)
        self._text_area.move_cursor((cursor_row, new_cursor_col))

    # Add these two methods:
    def get_cursor_position(self) -> int:
        """Get the current cursor position as a single integer offset.

        Returns:
            Integer offset from start of text
        """
        if not self._text_area:
            return 0
