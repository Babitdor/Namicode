...
class MentionController:
    """Controller for @ mentions (files and agents) with fuzzy matching."""
    ...
    def __init__(
        ...
        self._agent_cache: list[str] | None = None
    ...
    def _get_fuzzy_suggestions(self, search: str) -> list[tuple[str, str]]:
        """Get fuzzy suggestions (agents + files)."""
        suggestions: list[tuple[str, str]] = []
        search_lower = search.lower()
        
        # 1. Agents (Prioritized)
        # Only suggest agents if the search doesn't look like a file path (dots/slashes)
        # unless it explicitly matches an agent name
        if "/" not in search:
            agents = self._get_agents()
            # Simple prefix/fuzzy match for agents
            agent_matches = []
            for agent in agents:
                if search_lower in agent.lower():
                    # Score based on startswith
                    score = 100 if agent.lower().startswith(search_lower) else 50
                    agent_matches.append((score, agent))
            
            # Sort by score
            agent_matches.sort(key=lambda x: -x[0])
            
            for _, agent in agent_matches:
                suggestions.append((f"@{agent}", "agent"))
        
        # 2. Files
        files = self._get_files()
        # Include dotfiles only if query starts with "."
        include_dots = search.startswith(".")
        file_matches = _fuzzy_search(search, files, limit=MAX_SUGGESTIONS, include_dotfiles=include_dots)
...
# Alias for backwards compatibility (though new code should use MentionController)
FuzzyFileController = MentionController
PathCompletionController = MentionController
