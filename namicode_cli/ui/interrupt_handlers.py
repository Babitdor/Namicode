...
def resolve_plan_content(current_todos, dbg_func=None) -> tuple[str | None, Path | None]:
    ...
    try:
        ...
        nami_dir = settings.ensure_project_deepagents_dir()
        if not nami_dir:
            nami_dir = Path.cwd() / ".nami"

        # Case 1: agent wrote plan.md directly
        direct_plan = nami_dir / "plans" / "plan.md"
        ...
        if not plan_content and current_todos:
            ...
            nami_dir.mkdir(parents=True, exist_ok=True)
            plan_path = write_plan_file(current_todos, nami_dir)
