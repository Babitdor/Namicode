"""Dynamic context detection for Ollama models.

This module provides utilities to dynamically detect context window sizes
...
def get_model_config_dynamic(model_name: str) -> dict:
    """Get model configuration with dynamic context detection.
    ...
    Example:
        >>> config = get_model_config_dynamic("glm-5:cloud")
        >>> print(config)
        {
            "name": "glm-5",
...
def update_model_configs_dynamic() -> None:
    """Update model configurations with dynamic context detection.
    
    This function updates the MODEL_CONFIGS dictionary with actual
    context lengths from Ollama.
    
    Example:
        >>> update_model_configs_dynamic()
        >>> print(MODEL_CONFIGS["glm-5"].context_window)
        202752  # Actual value from Ollama
    """
    from namicode_cli.utils.model_config import MODEL_CONFIGS, ModelConfig
