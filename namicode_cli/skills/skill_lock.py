...
installed via `nami skills add`. Enables `nami skills update` to
re-fetch from the original source.

Lock file location:
  Global:  ~/.nami/skills-lock.json
  Project: {project_root}/.nami/skills-lock.json

The lock file sits *adjacent* to the skills/ directory (in the .nami dir),
not inside it.
"""
...
class SkillLock:
    ...
    def for_skills_dir(cls, skills_dir: Path) -> "SkillLock":
        ...
        Example: skills_dir = ~/.nami/skills/ → lock at ~/.nami/skills-lock.json
        """
