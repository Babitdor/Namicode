...
- **Removed outdated content**:
  - Deleted broken "Option 3: Install deepagents-nova separately" section
  - Removed non-functional "Docker Installation (Alternative)" section
  - Removed inaccurate "Terminal-Bench Evaluation" section with non-existent make targets
  - Removed aspirational Vixie Electron app description (replaced with actual server module)
- **Fixed CLI examples**:
  - Updated `--sandbox` examples to valid choices: `modal`, `daytona`, `runloop`, `docker`
  - Removed invalid `e2b` as a `--sandbox` value (E2B is a tool, not a sandbox backend)
  - Fixed `Nova doctor` → `nova doctor` (lowercase command)
- **Updated documentation**:
  - Expanded Built-in Tools table from ~15 to 25+ tools (added `think`, `duckduckgo_search`, `docs_search`, `convert_format`, `format_code_file`, `lint_code`, `check_types`, `http_request`, `package_info`, `get_current_time`, git tools, memory tools, `execute_in_e2b`, dev server tools, `run_tests`)
  - Fixed MCP presets to actual retained presets: `brave-search`, `memory`, `postgres`, `google-drive`, `playwright`, `fetch`, `time`, `sqlite`, `stripe`, `everything`, `serena`, `context7`
  - Replaced fictional module list (`agent.py`, `execution.py`, `config.py`, `tools.py`, `ui.py`) with actual `novacode_cli/` structure
  - Fixed Development Setup to use `make format` / `make lint` (ruff) instead of `black` / `isort`
  - Fixed path casing: `~/.Nova/` → `~/.nova/` throughout
  - Fixed Dependencies section text (removed "for windows" and "deepagents-Nova" inaccuracies)
- **Added new features**:
  - Added Voice Agent and Graphify Integration to Features list
  - Added version badge (0.0.19) near title

#### README.md Module Structure & Tools Update (2026-05-19)
- **Expanded Features list**: Added Project Graph, Plan Mode, Prompt Decomposition, LSP Integration, Semantic Code Search, Async Subagents, and Remote Bridges
- **Expanded Built-in Tools table**: Added 16 new tools — `query_project_graph`, `code_search`, `find_related_code`, 8 LSP tools (`lsp_goto_definition`, `lsp_find_references`, `lsp_hover`, `lsp_document_symbols`, `lsp_workspace_symbols`, `lsp_diagnostics`, `lsp_rename`, `lsp_signature_help`), and 5 async task tools (`start_async_task`, `check_async_task`, `update_async_task`, `cancel_async_task`, `list_async_tasks`)
- **Replaced Module Structure section**: Expanded from 14 flat entries to 42+ modules organized into 9 categories (Core, Agent, Commands, Configuration, Context & Memory, UI, Tools, Integrations, Infrastructure, Safety & Recovery, Specialized)

### Files Changed Summary

| File | Changes |
|------|---------|
| `README.md` | 20+ targeted fixes for accuracy and completeness |
