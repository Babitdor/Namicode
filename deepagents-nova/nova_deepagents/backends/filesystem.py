...
from nova_deepagents.backends.utils import (
    check_empty_content,
...
class FilesystemBackend(BackendProtocol):
    ...
    def read(
        ...
        Uses lazy line reading for efficient pagination - only reads
        the requested lines instead of the entire file.
        ...
        try:
            ...
            with os.fdopen(fd, "r", encoding="utf-8") as f:
                # Use lazy line reading - only reads needed lines
                # This is much more efficient for large files with pagination
                selected_lines = list(islice(f, offset, offset + limit))

            # Check for empty content
            if not selected_lines:
                # Check if file is empty or offset exceeds file length
                with open(resolved_path, "r", encoding="utf-8") as f:
                    ...
                    if not first_line:
                        ...
                        if empty_msg:
                            return empty_msg
                        return ""
                    # File has content, so offset must exceed file length
                    # Count lines to provide accurate error message
                    f.seek(0)
                    line_count = sum(1 for _ in f)
                    return f"Error: Line offset {offset} exceeds file length ({line_count} lines)"
    ...
    def write(
        ...
        Uses streaming writes for large content to reduce memory usage.

        Args:
            file_path: Path where the new file will be created.
            content: Text content to write to the file.
            chunk_size: Optional chunk size for streaming writes (bytes).
                       If None, writes entire content at once for small files.
                       Default: None (auto-detect based on content size).
        ...
        try:
            ...
            # Use streaming for large content (> 1MB)
            content_size = len(content.encode('utf-8'))
            use_streaming = chunk_size is not None or content_size > 1024 * 1024

            if use_streaming:
                # Stream content in chunks for better memory usage
                chunk = chunk_size or 1024 * 1024  # Default: 1MB chunks
                try:
                    with os.fdopen(fd, "w", encoding="utf-8") as f:
                        # Write in chunks to avoid memory issues with large content
                ...
                # Write entire content at once for small files
                with os.fdopen(fd, "w", encoding="utf-8") as f:
                    f.write(content)
    ...
    def edit(
        ...
        Uses efficient line-based editing for better performance on large files.
        Only reads and writes affected portions when possible.
        ...
        try:
            # For small files or multi-line replacements, use the original approach
            # For large files with single-line replacements, use streaming
            file_size = resolved_path.stat().st_size
            old_string_lines = old_string.count('\n') + 1

            # Use streaming for large files with single-line replacements
            if file_size > 1024 * 1024 and old_string_lines == 1 and not replace_all:
                return self._edit_streaming(resolved_path, file_path, old_string, new_string)

            # Standard approach for small files or multi-line replacements
            fd = os.open(resolved_path, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0))
            with os.fdopen(fd, "r", encoding="utf-8") as f:
                content = f.read()
    ...
    def _edit_streaming(
        ...
        """Edit a file using streaming for better performance on large files.

        Only reads and writes affected lines, not the entire file.
        Uses a temporary file for atomic write operation.
        ...
        Returns:
            EditResult with path and occurrence count.
        """
        import tempfile
        ...
        try:
            ...
            temp_fd, temp_path = tempfile.mkstemp(
                ...
                prefix='.tmp_edit_',
                suffix=str(resolved_path.suffix)
            )

            with os.fdopen(temp_fd, 'w', encoding='utf-8') as dst:
                with open(resolved_path, 'r', encoding='utf-8') as src:
                    for line in src:
                        if old_string in line:
                            # Replace first occurrence in line
                            modified_line = line.replace(old_string, new_string, 1)
                            dst.write(modified_line)
                            occurrences += 1

                            # If not replace_all, stop after first occurrence
                            # But continue copying rest of file
                            if occurrences == 1:
                                # Copy rest of file unchanged
                                for remaining_line in src:
                                    dst.write(remaining_line)
                                break
                        else:
                            dst.write(line)
    ...
    def _python_search(self, pattern: str, base_full: Path, include_glob: str | None) -> dict[str, list[tuple[int, str]]]:
        ...
        Uses streaming search for better memory efficiency on large files.
        Only reads one line at a time instead of entire file.
        ...
        for fp in walk_iter:
            ...
            # Use streaming search - read line by line instead of entire file
            try:
                    ...
                    for line_num, line in enumerate(f, 1):
                        ...
                        if regex.search(line):
                            if self.virtual_mode:
                                ...
                                virt_path = str(fp)
                            results.setdefault(virt_path, []).append((line_num, line.rstrip('\n')))
                            total_matches += 1
