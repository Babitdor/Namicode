...
def _get_memory_system_prompt(agent_memory: str) -> str:
    """Get the memory system prompt with the given agent memory content."""
    return render_template("memory.jinja", agent_memory=agent_memory)
...
class MemoryMiddleware(AgentMiddleware):
    ...
    def _format_agent_memory(self, contents: dict[str, str]) -> str:
        ...
        """
        if not contents:
            return _get_memory_system_prompt("(No memory loaded)")
        ...
        memory_body = "\n\n".join(sections)
        return _get_memory_system_prompt(memory_body)
