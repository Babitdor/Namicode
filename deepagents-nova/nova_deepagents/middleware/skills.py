...
if TYPE_CHECKING:
    from nova_deepagents.backends.protocol import BACKEND_TYPES, BackendProtocol
...
def _validate_skill_name(name: str, directory_name: str) -> tuple[bool, str]:
    ...
    logger.debug(
        "_validate_skill_name: Comparing name='%s' with directory_name='%s'",
        name,
        directory_name,
    )
...
def _parse_skill_metadata(
    ...
    # Ensure directory_name is just the basename (no path separators)
    normalized_dir_name = Path(directory_name).name

    is_valid, error = _validate_skill_name(str(name), normalized_dir_name)
    if not is_valid:
        logger.warning(
            "Skill '%s' in %s does not follow Agent Skills specification: %s. Consider reNovang for spec compliance.",
            name,
            skill_path,
            error,
        )
    ...
    if frontmatter_data.get("allowed-tools"):
        allowed_tools = frontmatter_data.get("allowed-tools").split(" ")  # type: ignore
    else:
        allowed_tools = []
...
def _list_skills(backend: BackendProtocol, source_path: str) -> list[SkillMetadata]:
    ...
    for (skill_dir_path, skill_md_path), response in zip(
        ...
        # Extract directory name from path using os.path.basename
        # Strip trailing separators (works on both Windows and Unix)
        directory_path = PurePosixPath(skill_dir_path)
...
class SkillsMiddleware(AgentMiddleware):
    ...
    def _get_backend(
        ...
        if callable(self._backend):
            ...
            backend = self._backend(tool_runtime)  # type: ignore
            if backend is None:
